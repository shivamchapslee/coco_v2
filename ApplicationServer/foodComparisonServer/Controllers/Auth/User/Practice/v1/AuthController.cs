...
namespace foodComparisonServer.Controllers.Auth.User.Practice.v1
{
    [Route("api/auth/user/v1")]
    [ApiController]
    public class AuthController : ControllerBase
    {
        ...
        #region CONSTRUCTOR
        public AuthController(IHostingEnvironment _HostingEnvironment, IConfiguration _Configuration, IHttpContextAccessor _HttpContextAccessor)
        {
        ...
        public IActionResult Authenticate([FromBody]PasswordLogin InputModel)
        {
            ...
            DataSet dsResponse = ObjSqlGateway.ExecuteSPWithDataSet("App_User_Login_Authentication", parameters);
            //DataTable dtResponse = ObjSqlGateway.bindDataTableParam("App_User_Login_Authentication", parameters);
            if (ObjHelper.CheckDBResponse(dsResponse))
            {
                ...
                if (drResponseRow["Response"] != null)
                {
                    if (drResponseRow["Response"].ToString() == "1") 
                    {
                        ...
                        ObjResponse.Data = ObjHelper.ConvertDatasetToDictionary(dsResponse);
                    }
                    else
                    {
